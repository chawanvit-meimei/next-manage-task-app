...
export default function Page() {
  ...
  const handleSaveClick = async () => {
    ...
    const {error: uploadError} = await supabase.storage.from('task_images').upload(new_filename, imageSelected);
    ...
    const {data} = await supabase.storage.from('task_bk').getPublicUrl(new_filename);
  ...
  return (
    <>
      ...
      >
        ...
        <h1 className="text-lg">เพิ่มงาน</h1>
